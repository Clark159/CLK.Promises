...
@implementation CPResultPromise


// fields
{
    
@private id(^_passResolved)(id);
...
// methods
- (CPPromise*) pushThen:(id(^)(id))onResolved onResolvedResultType:(enum ResultType)onResolvedResultType
    ...
    void(^resolveHandler)(id) = [^void(id result)
                                 {
                                     ...
                                     {
                                         // execute
                                         id resultObject = onResolved(result);
...
- (CPResultPromise*) pushThenNew:(id(^)(id))onResolved onResolvedResultType:(enum ResultType)onResolvedResultType
                      onRejected:(id(^)(NSException*))onRejected onRejectedResultType:(enum ResultType)onRejectedResultType
                      onNotified:(void(^)(CPProgress*))onNotified
{
    // contracts
...
{
    return [self
            pushThen:[^id(id result){ onResolved(result); return nil; } copy] onResolvedResultType:Empty
...
{
    return [self
            pushThen:[^id(id result){ return onResolved(result); } copy] onResolvedResultType:EmptyPromise
            onRejected:[self passRejected] onRejectedResultType:Empty
            onNotified:[self passNotified]
            ];
}
...
{
    return [self
            pushThenNew:[^id(id result){ return onResolved(result); } copy] onResolvedResultType:New
            onRejected:[self passRejected] onRejectedResultType:Empty
            onNotified:[self passNotified]
            ];
}
...
{
    return [self
            pushThenNew:[^id(id result){ return onResolved(result); } copy] onResolvedResultType:NewPromise
            onRejected:[self passRejected] onRejectedResultType:Empty
            onNotified:[self passNotified]
            ];
}
